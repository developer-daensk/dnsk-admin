...
const ContactPersonTable: React.FC = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    if (searchText) {
      ...
      result = result.filter(founder => {
        return (
          founder.userId.toLowerCase().includes(searchLower) ||
          founder.email.toLowerCase().includes(searchLower) ||
          founder.phone.toLowerCase().includes(searchLower) ||
          founder.mobile?.toLowerCase().includes(searchLower) ||
          founder.location.toLowerCase().includes(searchLower) ||
  ...
  const columns: ColumnsType<FounderData> = [
    {
      title: resourceHelpers.getCompanyContactsText('USER_ID'),
      dataIndex: 'userId',
      key: 'userId',
      width: 120,
    ...
    {
      title: resourceHelpers.getCompanyContactsText('LOCATION'),
      dataIndex: 'location',
      key: 'location',
      width: 120,
      sorter: (a, b) => a.location.localeCompare(b.location),
      filters: [
        ...Array.from(new Set(mockData.map(d => d.location))).map(location => ({
          text: location,
          value: location,
        })),
      ],
    ...
    {
      title: resourceHelpers.getCompanyContactsText('PHONE'),
      dataIndex: 'phone',
      key: 'phone',
      width: 140,
      render: text => (
    ...
    {
      title: resourceHelpers.getCompanyContactsText('MOBILE_PHONE'),
      dataIndex: 'mobile',
      key: 'mobile',
      width: 140,
      render: text => (
  ...
  const handleModalSuccess = () => {
    // Here you would typically refresh the data or add the new contact person to the list
    console.log('Contact person created successfully');
    // You could add the new contact person to the data array or refetch from API
  };
  ...
  return (
    <div>
      ...
      <BaseTable<FounderData>
        title={resourceHelpers.getCompanyContactsText('CONTACT_PERSONS')}
        columns={columns}
        data={filteredData}
        loading={loading}
        rowKey="userId"
        ...
        addButton={{
          text: resourceHelpers.getCompanyContactsText('ADD_CONTACT_PERSON'),
